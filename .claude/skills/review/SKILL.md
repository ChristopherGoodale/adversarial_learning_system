...
Everything the teach skill says about probing and about building a node still applies verbatim, including how to run a quiz-check and how to write the options. Do not reimplement either; use them.
...
Quiz-check down the ranked list, appending a ledger line after each one.
...
**Remember the option cap.** `AskUserQuestion` gives you 2–4 options, and "I don't know" takes one, so each question carries the correct answer plus two distractors. That is thin for diagnosis — so ask **more, smaller, adaptive questions** rather than trying to cram a full differential into one. Two sharp distractors asked three times tells you far more than six mediocre ones asked once.
...
For each node that actually broke, run the teach skill's Phase 3 loop unchanged: **motivate → establish → connect → quiz-check → record.**
...
The quiz-check at the end of each repair is what proves the node is back, and its ledger line is what lets the *next* review know. Do not skip either — an unconfirmed repair is indistinguishable from no repair, and an unrecorded one is indistinguishable from a node never tested.

## Step 5 — Housekeeping

- **Write ledger lines as you go**, never batched at the end. There is no extension capturing them for you.
...
- **Write the session to his note** like any lesson, so the repairs are re-readable later.
...
Catches a ledger line pointing at a node id that doesn't exist — usually a typo or a renamed id. It cannot catch a line you forgot to write, which is why Step 5's first bullet matters.
