---
name: synthesize
description: Teach a lesson that spans two subjects he has studied separately — applying maths to finance, connecting Chinese characters to software-engineering vocabulary, and so on. Finds the connection between two domain graphs, VERIFIES it is real before teaching it, then teaches the combined path. Use when he asks to combine, connect, bridge, or apply one subject to another.
---
...
- Both solid → proceed.
- One weak, unknown, or never quizzed → **quiz it now to find out.** Do not assume. If it is genuinely shaky, either repair it first (the review skill's Step 4) or pick a different bridge whose endpoints he does hold.
...
## Step 4 — Verify with the researcher, before teaching anything

Dispatch the `researcher` subagent on each candidate. Its existing contract is used unchanged — it returns Summary / Findings / Sources / Gaps.

Two things about the brief:

**It must be self-contained.** The researcher runs in an isolated context with no knowledge of this conversation. State both concepts fully, in plain terms, without leaning on anything said here.
...
Present the approach in prose alongside it, including **what you verified and what you rejected along the way.** He should see the bridge survived a check, not just that you asserted it.
...
Run the teach skill's Phase 3 loop. The bridge is a node like any other: **motivate → establish → connect → quiz-check.**

Its `connect` step is the one that matters most — the whole point is making the edge between the two domains explicit, so the connection is understood rather than filed as a coincidence. Tag every quiz with `subject` and `node` so outcomes reach the ledger.

## Step 7 — Write back

- **The bridge record** goes in `_bridges.md` with its verification result. That file is the single source of truth; never copy bridges into the domain graphs.
